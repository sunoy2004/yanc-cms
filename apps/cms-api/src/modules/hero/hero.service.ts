...
export class HeroService {
    ...
    try {
      ...
      if (data && data.id) {
        ...
        if (!mediaError && heroMediaItems) {
          ...
          // ONLY include items with Supabase Storage URLs (no Drive URL leakage possible)
          ...
          delete (transformedData as any).cta_text;
          delete (transformedData as any).cta_url;
    ...
    try {
      ...
      const { data: newHero, error: insertError } = await supabaseClient
        ...
        .insert({
          ...
          cta_url: dto.ctaLink || (dto as any).cta_url || '',
  ...
  async updateHeroContent(id: string, dto: UpdateHeroDto) {
    try {
      ...
      if ((dto as any).cta_url !== undefined) updateData.cta_url = (dto as any).cta_url;
  ...
  async deleteHeroContent(id: string) {
    try {
      ...
      // First, delete associated media items
      ...
      if (mediaDeleteError) {
        ...
        // Continue with hero deletion even if media deletion fails
      } else {
